...
( function($) {
	$.extend($.ui.spine.prototype, {
		...
		buildpackage:function(){
			...
			scriptArray = {
				scriptArray:[
				{
					...
					exc:function(){
						var _load, trackingrules, loaded, url;
						_load  = self.analytics_options.google.analytics.load_reporule;
						trackingrules = self.analytics_options.google.analytics.trackrule || {} ;
						loaded="";
						if(_load!==false){
							loaded="loading="+_load;
						}
						if(self.analytics_options.google.analytics.trackrule===false||_load!==false){
							url=("https:" === document.location.protocol ? "https://" : "http://") + "images.wsu.edu/javascripts/tracking/configs/pick.asp";
		...
		analytics_create: function(){
			...
			self=this;//hold to preserve scope
			self.buildpackage();
			scriptArray = self.analytics_options.scriptArray;
		...
		run_tracker:function(rules){
			var self, _DN, _CP;
			self=this;//hold to preserve scope
			_DN= self.analytics_options.google.analytics.domainName;
			_CP= self.analytics_options.google.analytics.cookiePath;
